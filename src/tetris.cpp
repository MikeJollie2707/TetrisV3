...
tick(0.5f)
{
    ...
    pieces.emplace_back();
    // next
    pieces.emplace_back(static_cast<Tetromino::Type>(genRandomTetromino()));
    // hint
    pieces.emplace_back();
}
Tetris::~Tetris()
...
int Tetris::genRandomTetromino() {
    ...
    else if (static_cast<int>(current->getType()) == value || value == 7) {
        value = rand() % 7;
    }
...
void Tetris::render() {
    ...
    shape.setSize(sf::Vector2f(grid - 2, grid - 2));
    ...
    Array5x5 const& render_next = next->getCurrentRotation();
    for (int i = 0; i < render_next.size(); ++i) {
        for (int j = 0; j < render_next[i].size(); ++j) {
            if (render_next[i][j] == 1) {
                sf::Vector2i const& current_pos = next->getPos();
                shape.setPosition(sf::Vector2f(
    ...
    if (hint != nullptr) {
        ...
        shape.setOutlineColor(getTetrominoColor(hint));
        shape.setOutlineThickness(outline);

        Array5x5 const& render_hint = hint->getCurrentRotation();
        for (int i = 0; i < render_hint.size(); ++i) {
            for (int j = 0; j < render_hint[i].size(); ++j) {
                if (render_hint[i][j] == 1) {
                    sf::Vector2i const& current_pos = hint->getPos();
                    shape.setPosition(sf::Vector2f(
                        (current_pos.x + (i - 2)) * grid,
    ...
    Array5x5 const& render = current->getCurrentRotation();
    for (int i = 0; i < render.size(); ++i) {
        for (int j = 0; j < render[i].size(); ++j) {
            if (render[i][j] == 1) {
                sf::Vector2i const& current_pos = current->getPos();
                shape.setPosition(sf::Vector2f(
                    (current_pos.x + (i - 2)) * grid,
                    (current_pos.y + (j - 2) - space_y) * grid
                ));
...
void Tetris::updateBoard() {
    if (!isMovable(current)) {
        Array5x5 const& pieces = current->getCurrentRotation();
        for (int i = 0; i < pieces.size(); ++i) {
            for (int j = 0; j < pieces[i].size(); ++j) {
                if (pieces[i][j] == 1) {
                    sf::Vector2i const& current_pos = current->getPos();
...
void Tetris::processEvent(sf::Event event) {
    ...
    if (type == sf::Event::Closed) {
        ...
        case sf::Keyboard::Right: {
            //rotateRight(current);
            rotateRight(pieces[0]);
            break;
        }
        case sf::Keyboard::Space: {
            if (settings.hard_drop) {
                while (isMovable(current) && !is_pause)
                    moveDown(current);
                is_hard_drop = true;
                movable = false;
            }
...
int Tetris::run() {
    while (window.isOpen()) {
        ...
        if (!movable)
        {
            ...
            pieces.emplace(pieces.begin() + 1, static_cast<Tetromino::Type>(genRandomTetromino()));
